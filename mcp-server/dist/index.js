...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "list_todos",
            description: "List all todo items from the Todo API. Returns id, body, and completed status for each.",
            inputSchema: {
                type: "object",
                properties: {},
            },
        ...
        switch (name) {
            case "list_todos": {
                ...
                const text = todos.length === 0
                    ? "No todos yet."
                    : todos
                        .map((t) => `- [${t.completed ? "x" : " "}] ${t.body} (id: ${t._id})`)
                        .join("\n");
            ...
            case "create_todo": {
                ...
                if (!body) {
                    return {
                        content: [{ type: "text", text: "Error: body is required and cannot be empty." }],
                        isError: true,
                    };
                }
                const created = await apiPost("/todos", { body, completed: false });
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "complete_todo": {
                ...
                return {
                    content: [{ type: "text", text: `Todo ${id} marked as completed.` }],
                };
            }
            case "delete_todo": {
                const id = String(safeArgs.id ?? "").trim();
                ...
                return {
                    content: [{ type: "text", text: `Todo ${id} deleted.` }],
                };
            }
            default:
                return {
